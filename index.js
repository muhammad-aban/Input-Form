...
btn.addEventListener("click",function(){
    ...
    if( name.value ==  "" || email.value == "" || phone.value == ""||plan.value == ""||billing =="" || street.value == ""|| city.value==""|| prov == ""||zipcode.value == ""|| country == ""|| password.value==""||confirmpas.value==""){
    ...
    if(/^[a-zA-Z0-9@]+$/.test(email.value)&&email.value.includes('@')/* &&email.value.includes('.com') */){
        ...
        fields = false
        error.innerHTML ="Email field does not contains a @ or number or charac"
    }
    if( !isNaN(phone.value) && phone.value.length == 13 ){
        ...
        fields = false
        error.innerHTML ="Phone field does not contain 13 digits or numbers"
    }
    if(/^\d{4}$/.test(zipcode.value)){
        ...
        fields = false
        error.innerHTML ="Zip field does not contain 4 and only digits"
    }
    if(/^[a-zA-Z]+$/.test(city.value)){
        ...
        fields = false
        error.innerHTML ="City field does not contain only alphabets"
    }
